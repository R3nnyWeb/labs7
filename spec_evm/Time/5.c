...
static PORT_InitTypeDef BtnInit;
//????????? ????????????? ?????
PORT_InitTypeDef PortStruct;
uint32_t global = 1000;
...
void ButtonsPinCfg(void){
	PORT_StructInit(&BtnInit);
	BtnInit.PORT_Pin = (PORT_Pin_5);
	
	BtnInit.PORT_OE = PORT_OE_IN;
	BtnInit.PORT_FUNC = PORT_FUNC_PORT;//порт в режиме стандартной функции
	BtnInit.PORT_MODE = PORT_MODE_DIGITAL;//цифровой режим порта
	BtnInit.PORT_SPEED = PORT_SPEED_SLOW;//скорость перехода сигнала от
	BtnInit.PORT_PULL_DOWN = PORT_PULL_DOWN_OFF;
	BtnInit.PORT_PULL_UP = PORT_PULL_UP_ON;
	BtnInit.PORT_GFEN = PORT_GFEN_ON;
	PORT_Init(MDR_PORTB, &BtnInit);
...
void  SysTick_Handler(void){
	global++;
	MDR_BKP->REG_00 = global;
	char globalStr[16];
	
	sprintf(globalStr, "%d", global);
	PrintString(globalStr,3);
...
//	BKP_RTC_ITConfig(BKP_RTC_IT_ALRF, ENABLE);
	...
	global = MDR_BKP->REG_00;
	while(1){
		if(!PORT_ReadInputDataBit(MDR_PORTB, PORT_Pin_5)){
			uint32_t time = 1701953615;
